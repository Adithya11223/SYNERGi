...
import {
  ...
  CheckCircle, AlertTriangle, XCircle, Info, ChevronRight, ArrowLeft,  Lock
} from 'lucide-react';
...
export function GeneralSettingsSection() {
  ...
  const [activeSection, setActiveSection] = useState<'profile' | 'password' | 'appearance' | 'privacy' | 'media' | 'personalization' | null>(null);
    ...
    <div className="space-y-6 max-w-3xl">
      {!activeSection && (
        <div className="space-y-6">
          <div className="glass-card rounded-2xl border overflow-hidden">
            <div onClick={() => setActiveSection('profile')} className="flex items-center justify-between p-4 border-b border-white/5 cursor-pointer hover:bg-white/5 transition-colors">
            ...
            <div onClick={() => setActiveSection('password')} className="flex items-center justify-between p-4 border-b border-white/5 cursor-pointer hover:bg-white/5 transition-colors">
              <div className="flex items-center gap-3">
            ...
            <div onClick={() => setActiveSection('appearance')} className="flex items-center justify-between p-4 border-b border-white/5 cursor-pointer hover:bg-white/5 transition-colors">
              <div className="flex items-center gap-3">
                <Monitor className="w-5 h-5 text-muted-foreground" />
                <span className="font-medium text-foreground">Appearance</span>
              </div>
              <div className="flex items-center gap-2 text-muted-foreground">
                <span className="text-sm">{settings.theme === 'system' ? 'System' : settings.theme === 'light' ? 'Light' : 'Dark'}</span>
                <ChevronRight className="w-4 h-4" />
              </div>
            </div>

            <div onClick={() => setActiveSection('privacy')} className="flex items-center justify-between p-4 border-b border-white/5 cursor-pointer hover:bg-white/5 transition-colors">
              <div className="flex items-center gap-3">
                <Shield className="w-5 h-5 text-muted-foreground" />
                <span className="font-medium text-foreground">Privacy & Status</span>
              </div>
              <div className="flex items-center gap-2 text-muted-foreground">
                <ChevronRight className="w-4 h-4" />
              </div>
            ...
            <div onClick={() => setActiveSection('media')} className="flex items-center justify-between p-4 border-b border-white/5 cursor-pointer hover:bg-white/5 transition-colors">
              <div className="flex items-center gap-3">
                <Volume2 className="w-5 h-5 text-muted-foreground" />
                <span className="font-medium text-foreground">Media & Sound</span>
              </div>
              <div className="flex items-center gap-2 text-muted-foreground">
                <ChevronRight className="w-4 h-4" />
              </div>
            ...
            <div onClick={() => setActiveSection('personalization')} className="flex items-center justify-between p-4 cursor-pointer hover:bg-white/5 transition-colors">
              <div className="flex items-center gap-3">
                <Palette className="w-5 h-5 text-muted-foreground" />
                <span className="font-medium text-foreground">Personalization</span>
              </div>
              <div className="flex items-center gap-2 text-muted-foreground">
                <ChevronRight className="w-4 h-4" />
              </div>
          ...
          <div className="glass-card rounded-2xl border overflow-hidden">
            <div onClick={() => logout()} className="flex items-center justify-center p-4 cursor-pointer hover:bg-destructive/10 transition-colors group">
      ...
      {activeSection && (
        <div className="flex items-center gap-4 mb-2">
          <Button variant="ghost" onClick={() => setActiveSection(null)} className="-ml-2 text-muted-foreground hover:text-foreground">
            <ArrowLeft className="w-4 h-4 mr-2" />
            Back
          </Button>
          <h2 className="text-xl font-bold tracking-tight text-white capitalize">
            {activeSection === 'privacy' ? 'Privacy & Status' : activeSection === 'media' ? 'Media & Sound' : activeSection === 'password' ? 'Password' : activeSection}
          </h2>
        </div>
      )}
              ...
              <Button 
                ...
                onClick={async () => {
                  ...
                  try {
                    ...
                    setPasswordError(err.errors?.[0]?.message || "Failed to update password. Make sure your current password is correct.");
                  } finally {
      ...
      <Card>
        <CardHeader>
          <CardTitle className="flex items-center text-lg"><User className="w-5 h-5 mr-2 text-primary" /> Edit Profile</CardTitle>
        ...
        <CardContent className="space-y-4">
          ...
          <div className="space-y-4 max-w-md">
            ...
            <Button 
              className="w-full mt-2" 
              disabled={profileLoading || !fullName}
              onClick={async () => {
                ...
                try {
                  ...
                  // Sync with backend so DB is updated
                  const token = await getToken();
                  const primaryEmail = clerkUser.primaryEmailAddress?.emailAddress;
                  if (primaryEmail && token && user) {
                  ...
                  // Update local auth store so UI instantly reflects changes
                  updateUser({
                    fullName: fullName
                  });
      ...
      <Card>
        ...
        <CardContent>
          <p className="text-sm text-muted-foreground">Password change options will appear here.</p>
        </CardContent>
      </Card>
      )}
      ...
      <Card>
        ...
        <CardContent className="space-y-0 md:space-y-6 flex flex-col">
          <div className="flex items-center justify-between px-4 md:px-0 py-3 md:py-0 bg-white/5 md:bg-transparent border-b border-white/5 md:border-none rounded-t-xl md:rounded-none">
            <div className="space-y-0.5">
              <Label className="text-base font-medium md:font-normal">Show Online Status</Label>
              <p className="text-sm text-muted-foreground hidden md:block">Let others see when you are active on the platform.</p>
            </div>
            <Switch checked={settings.showOnlineStatus} onCheckedChange={(v) => settings.updateSetting('showOnlineStatus', v)} />
          </div>
          <div className="space-y-2 md:space-y-4 pt-3 md:pt-4 px-4 md:px-0 pb-4 md:pb-0 bg-white/5 md:bg-transparent rounded-b-xl md:rounded-none md:border-t md:border-border">
            <div className="space-y-0.5">
      ...
      <Card>
        ...
        <CardContent className="space-y-0 md:space-y-6 flex flex-col">
          {[
            ...
            <div key={key} className={`flex items-center justify-between px-4 md:px-0 py-3 md:py-0 bg-white/5 md:bg-transparent ${idx !== arr.length - 1 ? 'border-b border-white/5 md:border-none' : ''} ${idx === 0 ? 'rounded-t-xl md:rounded-none' : ''} ${idx === arr.length - 1 ? 'rounded-b-xl md:rounded-none' : ''}`}>
              <div className="space-y-0.5">
                <Label className="text-base font-medium md:font-normal">{label}</Label>
                <p className="text-sm text-muted-foreground hidden md:block">{desc}</p>
              </div>
      ...
      <Card>
        ...
        <CardContent className="space-y-2">
          ...
          <div className="space-y-0 md:space-y-4 pt-1 flex flex-col">
            {[
