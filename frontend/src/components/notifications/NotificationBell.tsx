...
export const NotificationBell: React.FC = () => {
  const { notifications, markAsRead, markAllAsRead, deleteNotification, clearAll, dndMode, toggleDND } = useNotificationStore(useShallow(state => ({
  ...
  const handleSendReply = (e: React.MouseEvent | React.FormEvent, notif: AppNotification) => {
    ...
    try {
      ...
      if (client && client.connected) {
        ...
        // Remove the notification since we replied
        deleteNotification(notif.id);
      } else {
        ...
        {isOpen && (
          <>
            ...
            <motion.div
              ...
              className="absolute -left-4 md:left-auto md:right-0 mt-2 w-[calc(100vw-32px)] md:w-[380px] rounded-[1.5rem] shadow-[0_16px_40px_rgba(0,0,0,0.2)] dark:shadow-[0_16px_40px_rgba(0,0,0,0.6)] z-[100] overflow-hidden flex flex-col max-h-[80vh] border border-[var(--glass-border)]"
              style={{ background: 'var(--gb-floating)', backdropFilter: 'blur(var(--gb-blur-floating)) saturate(180%)', WebkitBackdropFilter: 'blur(var(--gb-blur-floating)) saturate(180%)' }}
            >
                      ...
                      >
                        ...
                        >
                          ...
                          <div className="bg-black/30 w-9 h-9 rounded-full shrink-0 flex items-center justify-center overflow-hidden">
                            {notif.avatarUrl ? (
                              <img src={notif.avatarUrl.startsWith('http') ? notif.avatarUrl : `http://localhost:1026${notif.avatarUrl}`} className="w-full h-full object-cover" alt="Avatar" />
                            ) : (
                              getIcon(notif.type)
                            )}
                          </div>
                          ...
                          <div className="flex-1 min-w-0 pr-6">
                            ...
                            <p className="text-[10px] text-muted-foreground/60 mt-1.5 flex justify-between items-center">
                              ...
                              <span>{safeFormatDistanceToNow(notif.createdAt)}</span>
                            </p>
                          </div>
