...
function NotificationPanel({
  ...
  onClose: () => void;
  onSendReply: (notification: StackedNotification, replyText: string) => void;
}) {
...
export default function GlobalChatBubble() {
  ...
  // ── Load initial and reconnected unread notifications
  useEffect(() => {
    if (user && isConnected) fetchNotifications();
  }, [user, isConnected]);
  ...
  useEffect(() => {
    ...
    const unsubscribe = subscribe(topic, (message: any) => {
      ...
      const newNotif: StackedNotification = { ...payload, id: payload.uuid };
      setNotifications((prev) => {
        if (prev.find((n) => n.id === payload.uuid)) return prev;
    ...
    // Subscribe to Phase 1 Sync Cleanup
    const syncTopic = `/topic/user.${user.uuid}.chat-notifications-sync`;
    const unsubscribeSync = subscribe(syncTopic, (message: any) => {
      const payload = JSON.parse(message.body);
      if (payload.notificationType === 'NOTIFICATIONS_READ') {
        setNotifications((prev) => 
          prev.filter(n => !(
            n.roomUuid === payload.roomUuid && 
            new Date(n.createdAt).getTime() <= new Date(payload.createdAt).getTime()
          ))
        );
      }
    });
  ...
  const fetchNotifications = async () => {
    try {
      ...
      setNotifications(data.map((n) => ({ ...n, id: n.uuid })));
    } catch (err) {
  ...
  const dismissNotification = (id: string) => {
    if (id.startsWith('group-')) {
      const roomUuid = id.replace('group-', '');
      const chatTypes = ['NEW_MESSAGE', 'IMAGE', 'DOCUMENT', 'VOICE_NOTE'];
      setNotifications((prev) => prev.filter((n) => !(n.roomUuid === roomUuid && chatTypes.includes(n.notificationType))));
    } else {
  ...
  const handleSendReply = async (notification: StackedNotification, text: string) => {
    if (!text.trim()) return;
    sendMessage(notification.roomUuid, text, notification.messageUuid);
    setReplyingTo(null);
    setReplyText('');
    
    try {
      // Phase 2: Leverage Phase 1 backend architecture to natively clear the notification
      await chatService.markAsRead(notification.startupUuid, notification.roomUuid, notification.messageUuid);
    } catch (err) {
      console.error("Failed to sync read receipt for reply", err);
    }
  };
  ...
  const groupedNotifications = useMemo(() => {
    const groups = new Map<string, StackedNotification[]>();
    const ungrouped: StackedNotification[] = [];

    notifications.forEach((notif) => {
      if (chatTypes.includes(notif.notificationType) && notif.roomUuid) {
        if (!groups.has(notif.roomUuid)) groups.set(notif.roomUuid, []);
        groups.get(notif.roomUuid)!.push(notif);
      } else {
        ungrouped.push(notif);
      }
    });

    const result: StackedNotification[] = [...ungrouped];

    groups.forEach((items, roomUuid) => {
      if (items.length === 1) {
        result.push(items[0]);
        return;
      }
      ...
      result.push({
        ...latest,
        id: `group-${roomUuid}`,
        senderName: latest.conversationName || latest.senderName,
        content: `${items.length} new messages`,
  ...
  const stackNotifs = groupedNotifications.slice(0, 3); // Show max 3 in stack
              ...
              {[...stackNotifs].reverse().map((n, revIdx) => {
                ...
                return (
                  <NotificationCard
                    ...
                    onSendReply={() => handleSendReply(n, replyText)}
                  />
                );
              })}
