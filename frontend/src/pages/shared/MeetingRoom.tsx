...
const VideoPlayer = ({ stream, isLocal, name, isMuted, isCameraOff, isScreenSharing }: any) => {
    ...
    return (
        <div className=" rounded-2xl overflow-hidden relative group h-full flex flex-col justify-center items-center">
            ...
            <div className="absolute bottom-4 left-4 /60 px-3 py-1.5 rounded-lg backdrop-blur-sm text-sm text-slate-900 dark:text-white flex items-center">
                {name} {isLocal ? '(You)' : ''}
...
export default function MeetingRoom() {
    ...
    return (
        <div className="flex flex-col h-[calc(100vh-130px)]  text-slate-900 dark:text-white rounded-xl overflow-hidden shadow-xl border border-gray-800 relative">
                        ...
                        <div className="flex-1 overflow-y-auto p-4">
                            {sidebarTab === 'participants' ? (
                                <div className="space-y-3">
                                    ...
                                    {participants.map(p => (
                                        <div key={p.uuid} className="flex items-center justify-between p-2 rounded-lg bg-foreground/5">
            ...
            {/* Top Bar (Title & Info) */}
            <div className="absolute top-4 left-4 bg-background/60 px-4 py-2 rounded-xl backdrop-blur-md flex items-center z-10 shadow-sm border border-border">
                <div className="w-2 h-2 rounded-full bg-red-500 animate-pulse mr-3"></div>
                <div>
                    <h3 className="font-bold text-sm text-slate-900 dark:text-white">{meeting.title}</h3>
            ...
            {/* Bottom Controls Bar */}
            <div className="h-24 border-t border-border bg-background/40 backdrop-blur-md flex items-center justify-between px-8 shrink-0 relative z-20">
                ...
                <div className="flex items-center space-x-4">
                    <button 
                        onClick={toggleAudio}
                        className={`w-14 h-14 rounded-full flex items-center justify-center transition-all shadow-lg ${isMuted ? 'bg-red-500 text-white' : 'bg-muted hover:bg-muted/80 text-foreground'}`}
                    >
                        {isMuted ? <MicOff className="w-6 h-6" /> : <Mic className="w-6 h-6" />}
                    </button>
                    <button 
                        onClick={toggleVideo}
                        className={`w-14 h-14 rounded-full flex items-center justify-center transition-all shadow-lg ${isCameraOff ? 'bg-red-500 text-white' : 'bg-muted hover:bg-muted/80 text-foreground'}`}
                    >
