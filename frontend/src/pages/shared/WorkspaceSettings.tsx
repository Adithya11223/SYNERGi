...
export default function WorkspaceSettings() {
  ...
  return (
    <div className="max-w-4xl mx-auto h-full flex flex-col pb-4 sm:pb-6 pt-2 space-y-4 sm:space-y-6 overflow-y-auto pr-2 custom-scrollbar">
      <div className="shrink-0">
        <h2 className="text-2xl font-bold text-slate-900 dark:text-white">Workspace Settings</h2>
        <p className="text-slate-900 dark:text-slate-600 dark:text-white/80 mt-1">Manage your startup profile, visibility, and members.</p>
      </div>
      ...
      <form onSubmit={handleSave} className="flex-1 flex flex-col glass-surface rounded-2xl shadow-sm border border-[var(--glass-border)] dark:border-gray-700 overflow-hidden">
        <div className="flex-1 overflow-y-auto p-4 sm:p-6 space-y-4 sm:space-y-6">
          <h3 className="text-lg font-semibold text-foreground dark:text-slate-900 dark:text-white mb-4">Basic Information</h3>
          ...
          <div className="space-y-4">
            ...
            <div className="glass-surface rounded-lg p-4 mt-6 border border-[var(--glass-border)] dark:border-gray-700">
              <h4 className="text-sm font-semibold text-foreground dark:text-slate-900 dark:text-white mb-3">Capacity Status</h4>
              <div className="grid grid-cols-2 gap-4 text-sm">
                <div>
                  <span className="block text-muted-foreground dark:text-muted-foreground">Current Approved Members:</span>
                  <span className="font-medium text-foreground dark:text-slate-900 dark:text-white">{data.approvedMembers || 0}</span>
                </div>
                <div>
                  <span className="block text-muted-foreground dark:text-muted-foreground">Available Slots:</span>
                  <span className="font-medium text-foreground dark:text-slate-900 dark:text-white">{Math.max(0, (data.maxMembers || 10) - (data.approvedMembers || 0))}</span>
                </div>
      ...
      <div className="glass-surface rounded-2xl shadow-sm border border-red-500/30 overflow-hidden relative">
        ...
        <div className="p-4 sm:p-6">
          ...
          <p className="text-sm text-slate-900 dark:text-slate-600 dark:text-white/70 mb-6">Irreversible and destructive actions for this workspace.</p>
