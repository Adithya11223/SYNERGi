...
export default function TeamChat() {
  ...
  const { messages, setMessages, setActiveRoom: setStoreActiveRoom, onlineStatuses, readReceipts, typingUsers, replyingTo, setReplyingTo, unreadCounts, roomActivities, resetUnreadCount, addOrUpdateMessage, updateMessageProgress } = useChatStore(useShallow((state: any) => ({
    ...
    roomActivities: state.roomActivities,
  ...
  const { uiMode, setChatSidebarAvailable, initiateCall } = useCallStore(useShallow((state: any) => ({
    uiMode: state.uiMode,
    setChatSidebarAvailable: state.setChatSidebarAvailable,
  ...
  const lastReadIndex = roomMessages.findIndex((m: any) => m.uuid === lastReadMsgUuid);
  ...
  // Issue 2 & 3: Proper Unread Clearing Logic
  // Only clear unread and send read receipts if:
  // 1. The document is focused and visible
  // 2. The user is at the bottom (seeing the latest messages)
  useEffect(() => {
    const handleVisibilityChange = () => {
      if (roomId && workspaceId && roomMessages.length > 0 && isAtBottom && document.hasFocus() && document.visibilityState === 'visible') {
        const unread = unreadCounts[roomId] || 0;
        // Check for newMsgCount as well, to clear the scroll badge
        if (unread > 0 || newMsgCount > 0) {
          const latestMsg = roomMessages.reduce((latest: any, current: any) => {
          ...
          if (latestMsg && latestMsg.uuid && !latestMsg.uuid.startsWith('temp-')) {
            markRead(roomId, latestMsg.uuid);
            // resetUnreadCount is now handled by the backend's syncReadWatermark event
            setNewMsgCount(0);
          }
        }
      }
    ...
    // Check initially and on dependencies change (e.g. new messages arrive, or user scrolls to bottom)
  ...
  }, [roomId, workspaceId, roomMessages, unreadCounts, isAtBottom, newMsgCount, resetUnreadCount]);
  ...
  const loadRoomAndMessages = useCallback(async (wId: string, rId: string) => {
    ...
    try {
      ...
      if (room.type === 'PRIVATE' && user) {
        ...
        const otherMember = members.find((m: any) => m.userUuid !== user.uuid);
        if (otherMember) {
      ...
      // Removed eager read receipt emission; the visibility observer useEffect will handle it safely.
  ...
  useEffect(() => {
    if (activeRoom) {
      setStoreActiveRoom(activeRoom.uuid, activeRoom.type === 'PRIVATE' ? (activeRoom as any).otherMemberUuid : null);
    } else {
      setStoreActiveRoom(null);
    }
  }, [activeRoom, setStoreActiveRoom]);
  ...
  useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
      ...
      if (e.key === 'ArrowUp' && !editingMessage && document.activeElement?.tagName !== 'TEXTAREA' && document.activeElement?.tagName !== 'INPUT') {
        const lastMyMsg = [...roomMessages].reverse().find((m: any) => m.senderUuid === user?.uuid && !m.isDeleted);
        if (lastMyMsg) {
          e.preventDefault();
    ...
    const membersList = workspaceMembers
      ...
      .map((m: any) => {
        ...
        return {
          ...
          isOnline: onlineStatuses[m.userUuid]?.isOnline ?? m.isOnline
        };
      });
  ...
  return (
    <div className="flex flex-1 h-full w-full bg-white/10 dark:bg-black/20 backdrop-blur-2xl border border-white/20 dark:border-white/5 shadow-2xl overflow-hidden rounded-[var(--radius)] relative">
      ...
      {roomId && activeRoom ? (
        <>
        <div id="chat-area-container" className="flex-1 flex flex-col h-full relative min-w-0">
          
          {/* Header */}
          <header className="h-16 shrink-0 px-4 md:px-6 flex items-center justify-between z-10 border-b border-border/50 glass-surface rounded-t-[var(--radius)] lg:rounded-t-none lg:rounded-tr-[var(--radius)]">
            <div className="flex items-center space-x-2 md:space-x-3 cursor-pointer">
              <Link to={`${user?.role === 'FOUNDER' ? '/founder' : '/talent'}/workspace/${workspaceId}/chat`} className="md:hidden p-1.5 mr-1 hover:bg-foreground/10 rounded-full text-foreground/80 transition-colors">
                <ArrowLeft className="w-5 h-5" />
              </Link>
              {activeRoom.type === 'PRIVATE' ? (
                <div className="relative shrink-0 w-10 h-10">
                  <img src={getImageUrl(activeRoom.otherMemberAvatarUrl) || `https://api.dicebear.com/7.x/initials/svg?seed=${activeRoom.name}&backgroundColor=000000`} alt={activeRoom.name} className="w-full h-full rounded-full object-cover border border-border" />
                ...
                <div className="relative shrink-0 w-10 h-10">
                  <img src={getImageUrl(activeRoom.iconUrl)} alt={activeRoom.name} className="w-full h-full rounded-full object-cover" />
                </div>
              ) : (
                <div className="relative shrink-0 w-10 h-10 rounded-full bg-primary/15 flex items-center justify-center text-primary font-bold">
              ...
              <div className="flex flex-col justify-center">
                ...
                <p className={`text-xs ${currentRoomActivities.length > 0 ? 'text-primary font-medium' : 'text-muted-foreground'}`}>
                  {currentRoomActivities.length > 0
                    ? currentRoomActivities.map((a: any) => `${a.name} is ${a.type.toLowerCase().replace('_', ' ')}...`).join(', ')
            ...
            {isLoadingMessages && page === 0 ? (
              ...
              {rowVirtualizer.getVirtualItems().map((virtualRow) => {
                ...
                if (msg.status === 'sending' || msg.status === 'failed') {
                  ...
                  const readCount = msg.statuses.filter((s: any) => s.readAt != null).length;
                  const deliveredCount = msg.statuses.filter((s: any) => s.deliveredAt != null || s.readAt != null).length;
                ...
                return (
                  ...
                  >
                    ...
                    <MessageBubble 
                      ...
                      onReplyClick={(replyToUuid) => {
                        const index = roomMessages.findIndex((m: any) => m.uuid === replyToUuid);
                        if (index !== -1 && rowVirtualizer) {
                          rowVirtualizer.scrollToIndex(index, { align: 'center' });
                          setHighlightedMessageId(replyToUuid);
        ...
        {isSearchSidebarOpen && (
          <ChatSearchSidebar 
            ...
            onJumpToMessage={(message) => {
              const index = roomMessages.findIndex((m: any) => m.uuid === message.uuid);
              if (index !== -1 && rowVirtualizer) {
                rowVirtualizer.scrollToIndex(index, { align: 'center' });
            ...
            <div className="p-4 space-y-4 max-h-[60vh] overflow-y-auto">
              ...
              <div>
                ...
                <div className="space-y-2">
                  {workspaceMembers.filter((m: any) => m.role !== 'OWNER' && m.role !== 'FOUNDER').length === 0 ? (
                    <div className="text-sm text-muted-foreground italic p-2">No team members available.</div>
                  ) : (
                    workspaceMembers.filter((m: any) => m.role !== 'OWNER' && m.role !== 'FOUNDER').map((m: any) => (
      ...
      />
      {uiMode === 'docked' && (
        <div className="hidden md:flex w-full sm:w-[320px] lg:w-[360px] h-full flex-shrink-0 bg-transparent flex-col transition-all duration-300 z-20">
