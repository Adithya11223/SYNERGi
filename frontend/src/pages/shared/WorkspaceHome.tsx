...
const WorkspaceHome: React.FC = () => {
  ...
  return (
    <div className="w-full max-w-full mx-auto h-full overflow-y-auto px-2 sm:px-4 md:px-8 pb-10">
      {/* Header Section */}
      <div className="glass-surface rounded-2xl shadow-sm border border-[var(--glass-border)] p-3 sm:p-5 md:p-8 mb-4 sm:mb-8 flex flex-col md:flex-row items-start md:items-center justify-between gap-3 sm:gap-4 md:gap-0">
        <div className="flex items-center space-x-6">
          ...
          <div>
            <div className="flex items-center space-x-3 mb-2">
              <h1 className="text-3xl font-bold text-white dark:text-slate-900 dark:text-white">{data.startupName}</h1>
            ...
            <p className="text-slate-900 dark:text-slate-600 dark:text-white/80 dark:text-muted-foreground text-lg max-w-2xl">{data.tagline}</p>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-3 gap-2 sm:gap-4 lg:gap-6 mb-4 sm:mb-8">
        <div className="glass-surface p-3 sm:p-4 lg:p-6 rounded-2xl shadow-sm border border-[var(--glass-border)] flex flex-col lg:flex-row items-start lg:items-center">
          ...
          <div>
            ...
            <h3 className="text-3xl font-bold text-foreground dark:text-slate-900 dark:text-white">{data.activeTaskCount}</h3>
          </div>
        </div>
        ...
        <div className="glass-surface p-3 sm:p-4 lg:p-6 rounded-2xl shadow-sm border border-[var(--glass-border)] flex flex-col lg:flex-row items-start lg:items-center">
          ...
          <div>
            ...
            <div className="flex items-baseline gap-2">
              <h3 className="text-3xl font-bold text-foreground dark:text-slate-900 dark:text-white">{data.teamMemberCount}</h3>
        ...
        <div className="col-span-2 sm:col-span-1 glass-surface p-4 lg:p-6 rounded-2xl shadow-sm border border-[var(--glass-border)] flex flex-col lg:flex-row items-start lg:items-center">
          ...
          <div>
            ...
            <h3 className="text-3xl font-bold text-foreground dark:text-slate-900 dark:text-white">{data.upcomingMeetings ?? 0}</h3>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-[2fr_1fr] gap-4 sm:gap-8">
        ...
        <div className="glass-surface rounded-2xl shadow-sm border border-[var(--glass-border)] overflow-hidden">
          <div className="p-4 sm:p-6 border-b border-[var(--glass-border)] dark:border-gray-700 flex justify-between items-center">
          ...
          <div className="p-4">
            {data.recentTasks.length === 0 ? (
              <p className="text-slate-900 dark:text-slate-600 dark:text-white/80 text-center py-6">No active tasks found.</p>
            ) : (
                ...
                {data.recentTasks.map(task => (
                  <li key={task.uuid} className="p-4 glass-surface/50 rounded-xl flex items-center justify-between group hover:bg-primary/10 dark:hover:bg-indigo-900/20 transition-colors cursor-pointer border border-transparent hover:border-primary/30 dark:hover:border-indigo-800">
                    <div className="flex flex-col">
                      <span className="font-medium text-foreground dark:text-slate-900 dark:text-white group-hover:text-primary dark:group-hover:text-indigo-300 transition-colors">{task.title}</span>
        ...
        <div className="glass-surface rounded-2xl shadow-sm border border-[var(--glass-border)] overflow-hidden flex flex-col">
          <div className="p-4 sm:p-6 border-b border-[var(--glass-border)] dark:border-gray-700 flex justify-between items-center bg-black/20">
                ...
                {workspaceNotifications.map(notif => (
                  <div key={notif.id} className="relative flex items-center justify-between md:justify-normal md:odd:flex-row-reverse group is-active">
                    ...
                    <div className="w-[calc(100%-4rem)] md:w-[calc(50%-2.5rem)] p-4 rounded-xl glass-card border border-white/5 shadow-md">
                      ...
                      <div className="text-xs text-slate-900 dark:text-slate-600 dark:text-white/70 line-clamp-2">{notif.body}</div>
                    </div>
                  </div>
                ))}
              </div>
