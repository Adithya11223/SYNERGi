...
export default function Profile() {
  ...
  if (loading) return (
    <div className="p-2 sm:p-6 lg:p-8 max-w-5xl mx-auto space-y-4 sm:space-y-6">
  ...
  return (
    <div className="p-2 sm:p-6 lg:p-8 max-w-5xl mx-auto space-y-4 sm:space-y-6">
      ...
      {/* Cover & Header */}
      <div className="relative md:mb-16 flex flex-col md:block items-center">
        <div 
          className="hidden md:block relative rounded-2xl overflow-hidden bg-gradient-to-r from-primary/20 via-primary/10 to-background border h-48 sm:h-64 bg-cover bg-center"
        ...
        <div className="md:absolute md:-bottom-12 md:left-8 flex flex-col md:flex-row items-center md:items-end gap-6 group z-10 mt-4 md:mt-0">
      ...
      <div className="flex flex-col md:flex-row justify-between items-center md:items-start gap-6 md:gap-4 px-2 text-center md:text-left mt-6 md:mt-0">
        <div className="w-full md:w-auto">
          <div className="flex flex-col items-center md:items-start">
            <div className="flex items-center gap-3 justify-center md:justify-start w-full">
              <h1 className="text-3xl font-bold tracking-tight text-slate-900 dark:text-white">{profile.user?.fullName || 'User Profile'}</h1>
              {profile.user?.role && (
            ...
            {profile.user?.username && (
              <p className="text-lg text-muted-foreground font-medium mt-1 w-full text-center md:text-left">@{profile.user.username}</p>
            )}
          </div>
          <div className="flex flex-col md:flex-row items-center gap-2 md:gap-4 mt-4 md:mt-3 text-sm text-muted-foreground">
            <span className="flex items-center gap-2"><MapPin className="w-4 h-4" /> {profile.profile?.city || 'No City'}, {profile.profile?.country || 'No Country'}</span>
            <span className="flex items-center gap-2"><Globe className="w-4 h-4" /> {profile.profile?.website || 'No Website'}</span>
          </div>
        </div>
        <div className="flex flex-col md:flex-row gap-4 md:gap-3 w-full md:w-auto mt-2 md:mt-0">
          ...
          {!isPublicView && (
            <Button onClick={() => setIsEditModalOpen(true)} className="w-full md:w-auto rounded-full md:rounded-full shadow-sm h-12 md:h-10 mt-2 md:mt-0 font-semibold text-base md:text-sm">
              <Edit className="w-5 h-5 md:w-4 md:h-4 mr-2" /> Edit Profile
            </Button>
          )}

          {isPublicView && user?.role === 'FOUNDER' && profile.user?.role === 'TALENT' && (
            <Button onClick={() => setIsHireModalOpen(true)} className="w-full md:w-auto rounded-full md:rounded-full shadow-sm bg-gradient-to-r from-primary to-blue-600 h-12 md:h-10 mt-2 md:mt-0 font-semibold text-base md:text-sm">
              <Briefcase className="w-5 h-5 md:w-4 md:h-4 mr-2" /> Hire Talent
            </Button>
          )}
        </div>
      </div>
      ...
      <div className="grid lg:grid-cols-3 gap-6 pt-6">
        <div className="lg:col-span-2 space-y-6">
          <Card>
            ...
            <CardContent>
              <p className="text-slate-900 dark:text-slate-600 dark:text-white/80 leading-relaxed whitespace-pre-wrap">{profile.profile?.bio || 'No overview provided.'}</p>
            </CardContent>
          </Card>
          ...
          <Card>
            ...
            <CardContent className="space-y-6">
              ...
              {(!profile.profile?.experience || profile.profile.experience.length === 0) && (
                <p className="text-slate-900 dark:text-slate-600 dark:text-white/80 text-sm">No experience listed.</p>
              )}
            </CardContent>
          </Card>
          ...
          <Card>
            ...
            <CardContent className="grid sm:grid-cols-2 gap-4">
              ...
              {(!profile.profile?.projects || profile.profile.projects.length === 0) && (
                <p className="text-slate-900 dark:text-slate-600 dark:text-white/80 text-sm col-span-2">No projects listed.</p>
              )}
            </CardContent>
          </Card>
        </div>
            ...
            <CardContent>
              <div className="flex flex-wrap gap-2">
                ...
                {(!profile.profile?.skills || profile.profile.skills.length === 0) && (
                  <p className="text-slate-900 dark:text-slate-600 dark:text-white/80 text-sm">No skills listed.</p>
                )}
              </div>
            </CardContent>
          </Card>
          ...
          <Card>
            ...
            <CardContent className="space-y-4">
              ...
              {(!profile.profile?.education || profile.profile.education.length === 0) && (
                <p className="text-slate-900 dark:text-slate-600 dark:text-white/80 text-sm">No education listed.</p>
              )}
            </CardContent>
          </Card>
