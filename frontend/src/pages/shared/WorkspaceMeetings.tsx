...
export default function WorkspaceMeetings() {
  ...
  const renderPlatformLogo = (id: string) => {
    switch (id) {
      case 'google-meet':
        return (
          <div className="w-12 h-12 md:w-20 md:h-20 glass-surface rounded-xl md:rounded-[24px] flex items-center justify-center shadow-sm border border-[var(--glass-border)] group-hover:scale-105 transition-transform duration-300 overflow-hidden">
            <img src="/google-meet-logo.png" alt="Google Meet" className="w-8 h-8 md:w-16 md:h-16 object-contain" />
          </div>
        );
      case 'ms-teams':
        return (
          <div className="w-12 h-12 md:w-20 md:h-20 bg-primary/5 rounded-xl md:rounded-[24px] flex items-center justify-center shadow-sm border border-[var(--glass-border)] group-hover:scale-105 transition-transform duration-300">
      ...
      case 'zoom':
        return (
          <div className="w-12 h-12 md:w-20 md:h-20 bg-blue-600 rounded-xl md:rounded-[24px] flex items-center justify-center shadow-sm group-hover:scale-105 transition-transform duration-300">
            <svg className="w-7 h-7 md:w-11 md:h-11 text-white" viewBox="0 0 24 24" fill="currentColor">
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 py-8 relative">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-10">
        <div>
          <div className="flex items-center text-3xl font-bold text-foreground dark:text-slate-900 dark:text-white mb-2">
          ...
          <p className="text-slate-900 dark:text-slate-600 dark:text-white/80 dark:text-muted-foreground text-[15px] font-medium">Connect with your team using your preferred meeting platform</p>
        </div>
      ...
      {/* Platform Cards */}
      <div className="grid grid-cols-2 lg:grid-cols-[repeat(auto-fit,minmax(280px,1fr))] gap-3 md:gap-6 mb-8 md:mb-12">
        {platforms.map(platform => (
          <div key={platform.id} className="glass-surface rounded-2xl md:rounded-[28px] p-4 md:p-6 border border-[var(--glass-border)] dark:border-gray-700 shadow-[var(--shadow-glass-sm)] flex flex-col hover:shadow-[var(--shadow-glass-lg)] transition-all duration-[var(--duration-smooth)] ease-[var(--ease-apple)] group">
            <div className="flex flex-col items-center text-center mb-4 md:mb-6">
              {renderPlatformLogo(platform.id)}
              <h3 className="text-sm md:text-xl font-bold text-foreground dark:text-slate-900 dark:text-white mt-2 md:mt-4 mb-1 md:mb-2 leading-tight">{platform.name}</h3>
              <p className="hidden md:block text-sm text-muted-foreground dark:text-muted-foreground px-1 leading-relaxed">{platform.description}</p>
            </div>

            <div className="hidden md:block flex-1 bg-primary/5 dark:bg-indigo-900/10 rounded-2xl p-5 mb-6 border border-gray-50 dark:border-indigo-900/20">
            ...
            <div className="space-y-2 md:space-y-3 mt-auto">
              <a 
                ...
                className="flex w-full items-center justify-center py-2 md:py-3 bg-primary hover:bg-primary/90 text-white font-semibold rounded-lg md:rounded-xl transition-colors shadow-sm text-xs md:text-sm"
              >
                <Plus className="w-3.5 h-3.5 md:w-4 md:h-4 mr-1.5 md:mr-2 opacity-80 hidden sm:inline-block" />
                Create
              </a>
              <button 
                onClick={() => {
                ...
                className="flex w-full items-center justify-center py-2 md:py-3 glass-surface border border-[var(--glass-border)] dark:border-gray-700 hover:border-primary/30 hover:text-primary dark:hover:border-indigo-800 dark:hover:text-primary text-muted-foreground dark:text-muted-foreground font-semibold rounded-lg md:rounded-xl transition-colors text-xs md:text-sm"
              >
                <ExternalLink className="w-3.5 h-3.5 md:w-4 md:h-4 mr-1.5 md:mr-2 opacity-70 hidden sm:inline-block" />
                Join
              </button>
            </div>
          </div>
      ...
      <div className="glass-surface rounded-[28px] p-8 mb-8 border border-border relative overflow-hidden">
        <h3 className="text-lg font-bold flex items-center text-foreground dark:text-slate-900 dark:text-white mb-8">
        ...
        <div className="fixed inset-0 z-[100] flex justify-end">
          ...
          <div className="relative w-full max-w-md glass-surface h-full shadow-2xl flex flex-col animate-in slide-in-from-right duration-300 border-l border-[var(--glass-border)] dark:border-gray-700">
            <div className="flex items-center justify-between p-6 border-b border-[var(--glass-border)] dark:border-gray-700 glass-surface sticky top-0 z-10">
              <div>
                <h2 className="text-xl font-bold text-slate-900 dark:text-white">Meeting History</h2>
                <p className="text-slate-900 dark:text-slate-600 dark:text-white/80 mt-1">Your legacy SYNERGi meetings</p>
              </div>
      ...
      {showJoinModal && (
        <div className="fixed inset-0 z-[110] flex items-center justify-center p-4">
          ...
          <div className="relative glass-surface w-full max-w-sm rounded-[24px] p-8 shadow-2xl animate-in zoom-in-95 duration-200">
            ...
            <h3 className="text-xl font-bold text-foreground dark:text-slate-900 dark:text-white mb-2">Join {selectedPlatform} Meeting</h3>
