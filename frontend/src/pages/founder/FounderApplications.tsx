...
export default function FounderApplications() {
  ...
  return (
    <div className="p-2 sm:p-6 lg:p-8 max-w-6xl mx-auto space-y-4 sm:space-y-8">

      <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-4">
        <div>
          <h2 className="text-3xl font-bold tracking-tight flex items-center gap-2 text-slate-900 dark:text-white"><Users className="w-8 h-8 text-primary" /> Applications</h2>
          <p className="text-slate-900 dark:text-slate-600 dark:text-white/80 mt-2">Review and manage talent applications for your startups.</p>
        </div>

        <div className="grid grid-cols-2 sm:flex sm:flex-row gap-3 sm:gap-4 w-full sm:w-auto mt-4 sm:mt-0">
            ...
            <SelectContent>
              {startups.map(s => (
                <SelectItem key={s.uuid} value={s.uuid}>
                  <span className="truncate block max-w-[120px] sm:max-w-none">{s.name}</span>
                </SelectItem>
              ))}
            </SelectContent>
          </Select>
          ...
          {selectedStartupId && startups.find(s => s.uuid === selectedStartupId)?.status === 'PUBLISHED' && (
            <Link to={`/founder/workspace/${selectedStartupId}`} state={{ from: location.pathname }} className="hidden sm:block">
          ...
          <Select value={filter} onValueChange={setFilter}>
            <SelectTrigger className="w-full sm:w-[160px] glass-card">
              <FilterIcon className="w-3 h-3 sm:w-4 sm:h-4 mr-1.5 sm:mr-2 shrink-0" />
      ...
      {!selectedStartupId ? (
        <div className="flex flex-col items-center justify-center py-20 px-4 text-center border rounded-2xl glass-card border-dashed">
          ...
          <p className="text-slate-900 dark:text-slate-600 dark:text-white/80">Please select a startup from the dropdown to view its applications.</p>
        </div>
      ) : loading ? (
          ...
          {filteredApps.map(app => (
            <Card key={app.uuid} className="hover:border-primary/50 transition-colors">
              <CardContent className="p-4 sm:p-6">
                ...
                <Link to={`/founder/applications/${app.uuid}`} className="block w-full">
                  <Button variant="outline" className="w-full bg-primary/5 hover:bg-primary/10 border-primary/20 text-primary transition-colors h-11 md:h-10">Review Application</Button>
                </Link>
              </CardContent>
            </Card>
          ))}
