...
import { Loader2, Trash2,  User as UserIcon, Calendar, ArrowRight, RotateCcw } from "lucide-react";
...
export default function FounderJobOffered() {
  ...
  if (loading) {
    return (
      <div className="p-2 sm:p-6 max-w-6xl mx-auto space-y-4 sm:space-y-8">
        <div>
  ...
  return (
    <div className="p-2 sm:p-6 max-w-6xl mx-auto space-y-4 sm:space-y-8">
      <div>
        <h1 className="text-3xl font-bold tracking-tight text-slate-900 dark:text-white mb-2">Job Offered</h1>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {offers.map(offer => (
            <Card key={offer.uuid} className="glass-card flex flex-col hover:border-primary/50 transition-colors">
              <CardHeader className="pb-4 relative flex flex-col items-center text-center pt-10">
                <div className="absolute top-4 left-4 z-10">
                  {getStatusBadge(offer.status)}
                </div>
                <div className="flex flex-col items-center w-full">
                  <div className="w-16 h-16 rounded-full bg-primary/10 overflow-hidden shrink-0 mb-3">
                    <img 
              ...
              <CardContent className="flex-1 space-y-4 flex flex-col items-center text-center px-6">
              ...
              <CardFooter className="flex gap-3 pt-4 border-t border-border/50 px-4 md:px-6 mt-auto">
                ...
                {offer.status === 'ACCEPTED' && (
                  <Button 
                    variant="outline"
                    className="w-full border-green-500/20 text-green-600 dark:text-green-400 bg-green-500/10"
                    disabled
                  >
                    Successfully Hired!
                  </Button>
