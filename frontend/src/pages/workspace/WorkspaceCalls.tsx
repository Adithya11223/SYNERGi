...
import { Phone, PhoneIncoming, PhoneOutgoing, PhoneMissed, PhoneOff, Clock, Calendar, Search, SearchX, PhoneCall, Filter } from 'lucide-react';
import {
  DropdownMenu,
  DropdownMenuContent,
  DropdownMenuItem,
...
export default function WorkspaceCalls() {
  ...
  return (
    <div className="flex flex-col bg-transparent max-w-5xl mx-auto w-full p-4 lg:p-8">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-4 sm:mb-6">
        <div>
          ...
          <p className="hidden md:block text-sm text-muted-foreground mt-1">Review your incoming and outgoing calls in this workspace.</p>
        </div>
        ...
        <div className="flex items-center gap-2 w-full md:w-auto">
           <div className="relative flex-1 md:w-64">
              ...
              <Input 
                 ...
                 className="pl-9 bg-background/50 border-border/50 h-9 text-sm rounded-lg"
              />
           </div>
           
           <DropdownMenu>
              <DropdownMenuTrigger asChild>
                 <button className="flex items-center justify-center shrink-0 w-9 h-9 rounded-lg border border-border/50 bg-background/50 hover:bg-white/5 transition-colors text-muted-foreground">
                    <Filter className="w-4 h-4" />
                 </button>
              </DropdownMenuTrigger>
              <DropdownMenuContent align="end" className="w-40 glass-surface">
                 {['ALL', 'INCOMING', 'OUTGOING', 'MISSED'].map(f => (
                    <DropdownMenuItem 
                       key={f}
                       onClick={() => setFilter(f as any)}
                       className={`flex items-center text-sm ${filter === f ? 'text-primary font-bold bg-primary/10' : 'text-foreground'}`}
                    >
        ...
        {isLoadingCalls || isLoadingMembers ? (
          ...
          <div className="space-y-2 pb-8">
            {visibleCalls.map((call, idx) => (
              <div 
                key={`${call.uuid}-${idx}`} 
                className="group flex flex-row items-center justify-between gap-2 sm:gap-3 p-2 sm:p-3 rounded-xl bg-card border border-border/50 hover:border-primary/30 transition-all hover:shadow-md"
              >
                 <div className="flex items-center gap-2.5 sm:gap-3 overflow-hidden">
                    <div className="relative shrink-0">
                      <Avatar className="w-9 h-9 sm:w-10 sm:h-10 border border-background">
                         <AvatarImage src={getImageUrl(call.peer.avatarUrl)} />
                         <AvatarFallback>{call.peer.fullName.charAt(0)}</AvatarFallback>
                      </Avatar>
                      <div className="absolute -bottom-0.5 -right-0.5 w-4 h-4 sm:w-4 sm:h-4 rounded-full bg-background flex items-center justify-center border border-border">
                         {getStatusIcon(call.status, call.isIncoming)}
                      </div>
                    </div>
                    <div className="flex flex-col min-w-0">
                       <span className="text-sm font-bold text-foreground truncate">{call.peer.fullName}</span>
                       <span className="text-[10px] sm:text-xs text-muted-foreground truncate">@{call.peer.username}</span>
                    </div>
                 </div>
                 ...
                 <div className="flex items-center gap-2 sm:gap-6 shrink-0">
                    <div className="flex flex-col items-end gap-0.5">
                       ...
                       <div className="flex items-center gap-1.5">
                          {call.durationSeconds > 0 && (
                            <span className="flex items-center gap-0.5 text-[10px] sm:text-xs font-mono text-muted-foreground">
                               <Clock className="w-2.5 h-2.5 sm:w-3 sm:h-3" />
                               {formatDuration(call.durationSeconds)}
                            </span>
                          )}
                          <span className={`px-1.5 py-0.5 rounded text-[8px] sm:text-[9px] font-bold tracking-wider uppercase ${getStatusColor(call.status)}`}>
                             {call.status}
                          </span>
                       </div>
                    </div>
                    ...
                    <button 
                       ...
                       className="shrink-0 w-8 h-8 sm:w-9 sm:h-9 rounded-full bg-primary/10 text-primary flex items-center justify-center hover:bg-primary hover:text-primary-foreground transition-all sm:opacity-0 sm:-translate-x-2 group-hover:opacity-100 group-hover:translate-x-0 focus:opacity-100 focus:translate-x-0"
