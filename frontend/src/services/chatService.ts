...
export const chatService = {
    ...
    getAttachments: async (startupUuid: string, roomUuid: string, type?: string, page = 0, size = 50) => {
        const params: any = { page, size };
        if (type) params.type = type;
        const response = await api.get<{ data: { content: AttachmentResponse[], totalPages: number, totalElements: number } }>(`/workspaces/${startupUuid}/chat/rooms/${roomUuid}/attachments`, { params });
        return response.data.data;
    },
