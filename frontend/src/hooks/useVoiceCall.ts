...
export function useVoiceCall() {
  ...
  const initPeerConnection = useCallback(() => {
    ...
    pc.oniceconnectionstatechange = () => {
      ...
      if (pc.iceConnectionState === 'checking' && callStateRef.current !== 'CONNECTED') setCallState('ICE_GATHERING');
      else if ((pc.iceConnectionState === 'connected' || pc.iceConnectionState === 'completed') && callStateRef.current !== 'CONNECTED') setCallState('ICE_CONNECTED');
    ...
    pc.onconnectionstatechange = () => {
      ...
      if (pc.connectionState === 'connecting' && callStateRef.current !== 'CONNECTED') {
          setCallState('CONNECTING');
          emitSignal('CALL_STATE_UPDATE', { state: 'CONNECTING' });
      }
      else if (pc.connectionState === 'connected') {
        // Do not set call state here! Just emit the signal and let the backend dictate the transition.
        emitSignal('CALL_CONNECTED');
        // Fallback: If we are already connected by the backend, or we were just connecting, 
        // we can set the start time if not already set.
        if (!useCallStore.getState().callStartTime) {
           useCallStore.getState()._set({ callStartTime: Date.now() });
        }
        
        // Also ensure state is set just in case the backend signal was missed
  ...
  const initiateCall = useCallback(async (peer: CallPeer, workspaceId: string, workspaceName?: string) => {
    ...
    useCallStore.getState().setManuallyMinimized(false);
  ...
  const initiateCallByUsername = useCallback(async (username: string, workspaceId: string) => {
    try {
        ...
        const res = await fetch(`http://localhost:1026/api/v1/users/search?username=${username}`, {
