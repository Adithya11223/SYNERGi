...
const SOCKET_URL = _apiBase.replace(/\/api\/v1\/?$/, '') + '/ws';
...
let globalSubscriptionsAdded = false;
  ...
  const ensureConnection = useCallback(async () => {
    ...
    connectionPromise = new Promise((resolve) => {
        ...
        client.onConnect = () => {
            ...
            // Automatically bulk-deliver any missed offline messages upon connection
            chatService.getUnreadNotifications().then((unread) => {
                const uniqueWorkspaces = [...new Set(unread.map((n: any) => n.startupUuid).filter(Boolean))];
                uniqueWorkspaces.forEach(uuid => {
                    apiClient.post(`/workspaces/${uuid}/chat/delivered/all`).catch(console.error);
                });
            }).catch(console.error);
            ...
            if (!globalSubscriptionsAdded) {
                globalSubscriptionsAdded = true;
        ...
        if (user) {
            const notifSub = globalStompClient.subscribe(`/topic/user.${user.uuid}.chat-notifications`, (message) => {
                try {
                    ...
                    if (body.type === 'syncReadWatermark') {
                        useChatStore.getState().resetUnreadCount(body.roomUuid);
                        return;
                    }

                    if (activeRoomUuid !== body.roomUuid || !document.hasFocus()) {
                        // Unread count is safely managed internally by addOrUpdateMessage now.
                        ...
                        // Send delivery receipt for background notification
                        if (globalStompClient?.connected && body.messageUuid && body.senderUuid && body.senderUuid !== user.uuid) {
                            globalStompClient.publish({
                                destination: `/app/chat.messageDelivered/${body.roomUuid}`,
                                body: JSON.stringify({ messageUuid: body.messageUuid })
                            });
                        }
                    } else {
                        // Issue 3: Eager markAsRead removed. TeamChat.tsx will safely emit the read receipt if the user is scrolled to the bottom.
                    }
                } catch(e) { console.warn(e); }
            });
                    ...
                    switch (body.type) {
                        ...
                        default:
                            ...
                            if (user && body.uuid && body.senderUuid && body.senderUuid !== user.uuid) {
                                ...
                                // Issue 3: Eager read receipt removed. TeamChat.tsx handles this securely.
                            }
                            break;
                    }
                } catch(e) { console.warn(e); }
