...
public interface ChatMessageRepository extends JpaRepository<ChatMessage, Long> {
    ...
    @org.springframework.data.jpa.repository.EntityGraph(attributePaths = {"sender", "room"})
    ...
    @org.springframework.data.jpa.repository.Query("SELECT m FROM ChatMessage m JOIN FETCH m.sender WHERE m.room = :room AND m.sender != :sender AND m.createdAt > :createdAt AND m.deleted = false ORDER BY m.createdAt DESC")
