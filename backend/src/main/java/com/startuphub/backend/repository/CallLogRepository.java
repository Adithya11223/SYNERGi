...
public interface CallLogRepository extends JpaRepository<CallLog, Long> {
    ...
    @Query("SELECT c FROM CallLog c JOIN FETCH c.caller JOIN FETCH c.receiver JOIN FETCH c.workspace WHERE c.workspace = :workspace AND ((c.caller = :user1 AND c.receiver = :user2) OR (c.caller = :user2 AND c.receiver = :user1)) ORDER BY c.startedAt DESC")
    ...
    @org.springframework.data.jpa.repository.EntityGraph(attributePaths = {"caller", "receiver", "workspace"})
