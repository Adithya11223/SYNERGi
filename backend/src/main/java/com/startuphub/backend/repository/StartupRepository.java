...
public interface StartupRepository extends JpaRepository<Startup, Long>, JpaSpecificationExecutor<Startup> {
    @EntityGraph(attributePaths = {"roles", "skills", "founder", "attachments"})
    ...
    @EntityGraph(attributePaths = {"founder", "attachments", "roles", "skills"})
    List<Startup> findByNameContainingIgnoreCaseOrTaglineContainingIgnoreCase(String name, String tagline);
    
    @Query("SELECT s FROM Startup s JOIN FETCH s.founder WHERE (LOWER(s.name) LIKE LOWER(CONCAT('%', :query, '%')) OR LOWER(s.tagline) LIKE LOWER(CONCAT('%', :query, '%'))) AND s.status = :status AND s.isDeleted = false")
    ...
    @EntityGraph(attributePaths = {"founder", "roles", "skills"})
