...
public class ChatController {
    ...
    private final UrlSignerService urlSignerService;
    ...
    public ResponseEntity<ApiResponse<org.springframework.data.domain.Page<AttachmentResponse>>> getRoomAttachments(
            @PathVariable UUID startupUuid,
            @PathVariable UUID roomUuid,
            ...
            Authentication authentication) {
        return ResponseEntity.ok(ApiResponse.success(
                chatMessageService.getRoomAttachments(authentication.getName(), startupUuid, roomUuid, type, pageable),
    ...
    public ResponseEntity<Resource> downloadAttachment(
        ...
        if (!urlSignerService.validateSignature(path, expires, signature)) {
    ...
    @GetMapping("/rooms/{roomUuid}/attachments/{attachmentUuid}/refresh")
    public ResponseEntity<ApiResponse<String>> refreshAttachmentUrl(
            @PathVariable UUID startupUuid,
            @PathVariable UUID roomUuid,
            @PathVariable UUID attachmentUuid,
            Authentication authentication) {
        ...
        try {
            String signedUrl = chatMessageService.getAttachmentSignedUrl(authentication.getName(), startupUuid, roomUuid, attachmentUuid);
            return ResponseEntity.ok(ApiResponse.success(signedUrl, "URL refreshed successfully"));
        } catch (Exception e) {
            return ResponseEntity.status(org.springframework.http.HttpStatus.FORBIDDEN).build();
        }
    }
    ...
    public ResponseEntity<ApiResponse<Void>> markAllAsDelivered(
            @PathVariable UUID startupUuid,
            Authentication authentication) {
        chatMessageService.markAllAsDelivered(authentication.getName(), startupUuid);
        return ResponseEntity.ok(ApiResponse.success(null, "Offline messages marked as delivered"));
    }
}
